...
public class EnviromentGenAlgorithms
{
    ...
    public static void GenerateDefaultTree(Block[,,] worldBlockData, Vector3 rootPosition)
    {
        ...
        {
            if (WorldGenerateUtils.CheckSubWorldBoundary((int)rootPosition.x, (int)rootPosition.y + idx, (int)rootPosition.z) == true)
            {
    ...
    private static void MakeDefaultBranch(Block[,,] worldBlockData, Vector3 branchPos, int depth, BlockTileType leafType)
    {
        ...
        for (int x = -1; x < 2; x++)
        {
            for (int y = -1; y < 2; y++)
            {
                for (int z = -1; z < 2; z++)
                {
                    if (WorldGenerateUtils.CheckSubWorldBoundary((int)branchPos.x + x, (int)branchPos.y + y, (int)branchPos.z + z) == true)
                    {
        ...
        if (WorldGenerateUtils.CheckSubWorldBoundary((int)branchPos.x, (int)branchPos.y + 1, (int)branchPos.z) == true) MakeDefaultBranch(worldBlockData, new Vector3(branchPos.x, branchPos.y + 1, branchPos.z), depth - 1, leafType);
        if (WorldGenerateUtils.CheckSubWorldBoundary((int)branchPos.x + 1, (int)branchPos.y + 1, (int)branchPos.z) == true) MakeDefaultBranch(worldBlockData, new Vector3(branchPos.x + 1, branchPos.y + 1, branchPos.z), depth - 1, leafType);
        if (WorldGenerateUtils.CheckSubWorldBoundary((int)branchPos.x - 1, (int)branchPos.y + 1, (int)branchPos.z) == true) MakeDefaultBranch(worldBlockData, new Vector3(branchPos.x - 1, branchPos.y + 1, branchPos.z), depth - 1, leafType);
        if (WorldGenerateUtils.CheckSubWorldBoundary((int)branchPos.x, (int)branchPos.y + 1, (int)branchPos.z + 1) == true) MakeDefaultBranch(worldBlockData, new Vector3(branchPos.x, branchPos.y + 1, branchPos.z + 1), depth - 1, leafType);
        if (WorldGenerateUtils.CheckSubWorldBoundary((int)branchPos.x, (int)branchPos.y + 1, (int)branchPos.z - 1) == true) MakeDefaultBranch(worldBlockData, new Vector3(branchPos.x, branchPos.y + 1, branchPos.z - 1), depth - 1, leafType);
    }
